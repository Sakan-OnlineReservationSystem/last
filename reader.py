...
class MultiWozReader(_ReaderBase):
    ...
    async def down(self):
        output_path = cfg.data_path + cfg.data_file
        download_url = "https://drive.google.com/file/d/1spkaxn4xp5RLhVOLbloNJltz9IiR28jC/view?usp=sharing"
        # Ensure the cfg.gpt_path directory exists
        if not os.path.exists(cfg.data_path):
            os.makedirs(cfg.data_path)
        try:
            await gdown.download(download_url, output_path, quiet=False)
            # Check if the file is downloaded successfully
            # Set the desired duration in seconds (1 minute = 60 seconds)
            if not os.path.isfile(output_path):
                raise FileNotFoundError(f"Download failed, file not found: {output_path}")
        except Exception as e:
            print(f"Failed to download the file: {e}")
            # Handle the failure as appropriate (retry, exit, etc.)
            return
        self.data = json.loads(
            open(output_path, 'r', encoding='utf-8').read().lower())
        self.train, self.dev, self.test = [], [], []
    ...
    def _load_data(self, save_temp=False):
        if save_temp: # save encoded data
            ...
            if os.path.exists(encoded_file):
                ...
                #
                output_path = cfg.slot_value_set_path
                try :
